    ...
    return function (from, to, title, msg, file, callback) {
        if (to.substr(0, 3) === "+82") {
            if (infoBankSMSConfig && infoBankSMSConfig.mmsUse == "true" && infoBankSMSConfig.serviceId && infoBankSMSConfig.servicePw) { // 인포뱅크
                ...
                if (globalInfoBankAuth && globalInfoBankAuth.expired) {
                    // 인포뱅크 기준시간은 KST(UTC+9)이고 토큰 유효기간은 24시간
                    var expired = globalInfoBankAuth.expired;
                    expired = new Date(expired.slice(0, 4), expired.slice(4, 6) - 1, expired.slice(6, 8), expired.slice(8, 10), expired.slice(10, 12), expired.slice(12, 14));

                    if (expired.getTime() - 43200000 < new Date().getTime()) {
                        globalInfoBankAuth = null;
                    } else {
                        infoBankAuth = globalInfoBankAuth;
                    }
                }
